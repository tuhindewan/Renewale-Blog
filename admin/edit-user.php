               ...
               $image = $e_image;
                  ...
                  $insert_password = crypt($password,$salt);
                     ...
                     if (empty($first_name) or empty($last_name) or empty($image)) {
                        ...
                        {
                         $statement = $db->prepare("UPDATE  `cms`.`users` SET  `first_name` =  '$first_name',`last_name` =  '$last_name',`image` =  '$image',`role` =  '$role',`details` =  '$details' WHERE  `users`.`id` =$edit_id");
                         if (isset($password)) {
                           $statement = $db->prepare("UPDATE  `cms`.`users` SET  `first_name` =  '$first_name',`last_name` =  '$last_name',`image` =  '$image',`role` =  '$role',`details` =  '$details',`password` =  '$insert_password' WHERE  `users`.`id` =$edit_id");
                         }
                         $statement->execute();
              ...
              <form action="" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                  ...
                  <select name="role" id="role" class="form-control">
                    ...
                    <option <?php if($e_role == 'admin'){echo "selected";} ?> value="admin">Admin</option>
                  </select>
                </div>
